...
- Seller, buyer và các trang nội dung dùng chung `src/components/SiteFooter.astro`; page shell dùng `min-h-dvh` cùng flex column để footer bám cuối viewport khi nội dung ngắn. Footer hiển thị số điện thoại, email và LinkedIn inline với icon SVG, còn footer liên hệ trên homepage giữ cùng cách trình bày và cũng hiển thị LinkedIn.
...
- Trang `/about/` và `/en/about/` đã được tạo bằng component dùng chung với hero ảnh `archway_business_people_reviewing_financial_documents_1`, overlay navy đậm, heading rộng, section đội ngũ hai cột với heading navy nhỏ hơn bên trái cùng body rộng khoảng 65% bên phải, section `Chúng tôi là ai` ba cột kèm sứ mệnh cùng tầm nhìn, rồi section founder có ảnh chân dung và lời nhắn ngắn trong khung nền navy đậm; buyer, contact, how-it-works và privacy policy dùng chung `src/components/PageHero.astro` với phong cách hero này; các route `/industries-we-serve/` và `/en/industries-we-serve/` dùng component dùng chung với sáu card ngành con có ảnh responsive và liên kết đến trang chi tiết.
- Các route ngành chi tiết giữ cùng slug giữa hai ngôn ngữ để tương thích với canonical, hreflang và chuyển ngôn ngữ hiện tại. Cả sáu ngành dùng `src/components/IndustryServicePage.astro` và `src/data/industry-service-pages.ts`; trang tập trung vào dịch vụ đại diện bên bán, phạm vi công việc, benchmark định giá riêng theo ngành, ví dụ minh họa, kết quả thương mại, bảo mật và lợi ích cho chủ doanh nghiệp, không giải thích quy trình Archway theo kiểu bài hướng dẫn người mua, không dùng lưới card dày đặc, eyebrow hoặc số trang trí. Mỗi trang có CTA sớm và CTA cuối dẫn trực tiếp đến form `sell-your-business`. CTA chính dùng `Định giá miễn phí` hoặc `Get a Free Business Valuation`.

## Tài sản tĩnh được xử lý trước khi build.

- `public/assets/image-library/` chứa 53 ảnh cùng các bản AVIF và WebP ở các chiều rộng phù hợp với từng nguồn, chủ yếu là 640px, 1280px và 1920px; `manifest.json` lưu metadata. Ảnh HVAC `archway_hvac_ductwork` cùng năm ảnh ngành cho nhà thầu điện, logistics, máy móc công nghiệp, IT và thiết bị y tế đã được tải về và xử lý thành các bản AVIF và WebP responsive. Ảnh chăm sóc người cao tuổi từ `image_source/common_use/dich-vu-cham-soc-nguoi-cao-tuoi-1024x683.jpg` đã được xử lý thành AVIF và WebP 640px cùng 1024px và được giữ lại trong thư viện cho các trang healthcare tương lai. Không xoá hoặc di chuyển thư mục này vì đây là thư viện dành cho các trang tương lai.
...
- `public/llms.txt` mô tả dịch vụ, tiêu chí, biểu mẫu, LinkedIn và sáu ngành trọng tâm cùng toàn bộ URL ngành song ngữ.
...
- `SiteLayout` dùng structured data `Organization` kết hợp `ProfessionalService`, liên kết hồ sơ LinkedIn bằng `sameAs`, còn About và Industries lần lượt dùng kiểu `AboutPage` và `CollectionPage`; trang ngành chi tiết bổ sung node `Service` trong JSON-LD.
...
- Các route tiếng Việt và tiếng Anh đã có keyword mapping theo ý định tìm kiếm; homepage giữ keyword rộng, seller giữ ý định bán, buyer giữ ý định mua, how-it-works giữ ý định tìm hiểu quy trình, About giữ entity và trust, Industries giữ phạm vi ngành, trang ngành chi tiết giữ ý định bán theo từng ngành và Contact giữ ý định liên hệ.
